...
import {
  ...
  STATUS_ASET,
  logAsetTersaring,
  type SaringanLogAset,
} from "@/lib/aset";

/** Saringan log perubahan aset, disimpan di URL agar bisa dibagikan. */
export function SaringLogAset({
  ...
  const aktif = logAsetTersaring(saringan);
      ...
      <div className="space-y-3 px-5">
        <div className="relative">
          ...
          <input
            ...
            maxLength={60}
            autoComplete="off"
            aria-label="Cari log perubahan aset"
            placeholder="Cari aset, pemegang, atau pencatat"
        ...
        <p className="flex flex-wrap items-center justify-between gap-2 text-[11px] leading-[14px] text-muted-foreground">
          <span>
            {aktif ? `${jumlah} dari ${total} kejadian` : `${total} kejadian`}
          </span>
          {aktif ? (
            <button
              type="button"
